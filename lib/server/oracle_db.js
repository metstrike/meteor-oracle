...
OracleDB.getDatabase = function(oracleOptions) {
	var connection = oracleOptions.connection;
...
OracleDB.prototype._refreshTable = function(sqlTable, oracleOptions) {
	  ...
	  self._refreshTrigger(sqlTable, oracleOptions);
}

OracleDB.prototype._refreshTrigger = function(sqlTable, oracleOptions) {
	  var self = this;
	  ...
	  var ns = oracleOptions.connection.user+'@'+oracleOptions.connection.connectString+'.'+sqlTable; 
	  ...
	  trgSql = trgSql + '\t\tinsert into "oplog" ("id", "ts", "tr", "v", "op", "ns", "o", "o2")\n'
	  trgSql = trgSql + '\t\tvalues ("oplog_seq".nextval, current_timestamp, dbms_transaction.local_transaction_id, 2, op, ns, o, o2);\n';	  
...
OracleDB.prototype._ensureTable = function(sqlTable, oracleOptions, parentSqlTable) {
	  ...
	  if(!self._tables[sqlTable]) {
		  ...
		  if(!self._tables[sqlTable]) {
		  	  if(oracleOptions.strict || oracleOptions.strictAllowOplogAndTriggers) {
		  		  throw new Error("Table \"" + sqlTable + "\" does not exist and can't be created in strict mode");
		  	  }
...
OracleDB.prototype._ensureOplog = function(oracleOptions) {
	  var self = this;
	  var sqlTable = "oplog";
	  ...
	  if(!self._tables[sqlTable]) {
		  ...
		  if(!self._tables[sqlTable]) {
	  		  // Oplog doesn't exist, we should create it
	  		  var sql;
	  		  var result;
	
	  		  // Create table
    		  sql = 'create table "oplog" \n\
( \n\
"id" number not null, \n\
...
"o2" varchar2(4000) null \n\
	    	  ...
	    	  result = self.executeCommand(sql, []);
	  		  ...
	  		  // Add primary key
    		  sql = 'alter table "oplog" add constraint "oplog_pk" primary key ("id")';
	    	  ...
	    	  result = self.executeCommand(sql, []);
	  		  ...
	  		  // Add index
    		  sql = 'create index "oplog_i1" on "oplog" ("ts")';
	    	  ...
	    	  result = self.executeCommand(sql, []);
	  		  ...
	  		  // Create package
    		  sql = 'create or replace package meteor_pkg is \n\
\n\
...
\n\
	    	  ...
	    	  result = self.executeCommand(sql, []);
	  		  ...
	  		  // Create package body
    		  sql = 'create or replace package body meteor_pkg is \n\
\n\
